...
const NewTicketTypeModal = ({
  modalVisible,
  setModalVisible,
  featured_event_id,
  fetchTicketTypes
}: NewTicketTypeModalProps) => {
  ...
  const currentUser = useSelector(selectCurrentUser);
    ...
    sales_end: new Date(Date.now() + 14400000), // 4 hours later
  });
    const handleCreateTicket = async () => {
        
        if (!newTicket.is_free && newTicket.price === '0') {
            platformAlert('Please select a price');
            return;
        }

        if (Number(newTicket.quantity) <= 4) {
            platformAlert('The available quantity must be at least 5');
            return;
        }
        if (Number(newTicket.price) < 3 && !newTicket.is_free) {
            platformAlert('Paid tickets must cost at least £3');
            return;
        }
        if (newTicket.sales_start >= newTicket.sales_end) {
            platformAlert('Sales start date must be before sales end date');
            return;
        }

        if (newTicket.sales_start <= new Date(Date.now() - 86400000)) {
        ...
        try {
          ...
          const organiser_id = await fetchOrganizerId(currentUser?.id);
          const { error } = await supabase
              ...
              .insert({
                  ...
                  is_free: newTicket.price === '0' || newTicket.is_free ? true : false,
                  price: newTicket.price,
                  quantity: newTicket.quantity,
                  sales_start: newTicket.sales_start,
                  sales_end: newTicket.sales_end
              })
          if (error) {
          ...
          fetchTicketTypes();
          platformAlert('New ticket type created successfully');
        } catch (error) {
          console.error(error);
        } finally {
          setNewTicket({
    ...
    >
      <View className="flex-1 bg-zinc-100">
        ...
        >
          ...
          <View className="bg-white rounded-3xl p-2 flex-row mb-5">
            ...
            <TouchableOpacity
              ...
              className={`flex-1 rounded-2xl py-4 items-center ${!newTicket.is_free
                ? "bg-blue-600"
                : ""
                }`}
            >
              <Text
                className={`font-semibold ${!newTicket.is_free
                  ? "text-white"
                  : "text-zinc-600"
                  }`}
              >
          ...
          <View className="bg-white rounded-3xl p-3 space-y-1">
            ...
            <View className="flex-row gap-3">
              ...
              <View
                className={`flex-1 ${newTicket.is_free
                  ? "opacity-40"
                  : ""
                  }`}
              >
            ...
            <View className="bg-white rounded-3xl p-5 mt-5">
              ...
              >
                <View>
                  ...
                  <Text className="text-zinc-500">
                    {formatDateShortWeekday(newTicket.sales_start)}
                  </Text>
                </View>
                <View className="flex-row items-center space-x-2">

                  <Text className="text-zinc-800">
                    {extractTimeFromDateSubmit(newTicket.sales_start)}
                  </Text>
              ...
              >
                <View>
                  ...
                  <Text className="text-zinc-500">
                    {formatDateShortWeekday(newTicket.sales_end)}
                  </Text>
                </View>
                <View className="flex-row items-center space-x-2">

                  <Text className="text-zinc-800">
                    {extractTimeFromDateSubmit(newTicket.sales_end)}
                  </Text>
                  <AntDesign
                    name="calendar"
                    size={18}
        ...
        <DatePicker
          modal
          open={openSalesStart}
          date={new Date(newTicket.sales_start)}
          onConfirm={(date) => {
        ...
        <DatePicker
          modal
          open={openSalesEnd}
          date={new Date(newTicket.sales_end)}
          onConfirm={(date) => {
...
export default NewTicketTypeModal